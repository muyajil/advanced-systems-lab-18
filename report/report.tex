...
\par
%
Now if we look at figure~\ref{fig:one_middleware_set_tp_mw} we can see that the system is saturated when using 24 clients with 8 threads, 96 clients with 16 threads, XX\todo{real number} clients using 32 threads and XX\todo{real number} with 64 worker threads.
%
...
%
At this point the bottleneck moves to the middleware.
\par
...
%
This is again due to the nature of \texttt{GET} requests, since we need to retrieve the value from memory and not only store it in constant time. 
%
\par
%
It might be a bit misleading because a \texttt{GET} request does not produce more work for the middleware than \texttt{SET} requests.
%
Therefore one might think that the middleware would be able to handle at least the same amount of \texttt{GET} requests as \texttt{SET}, since the middleware does not do more "work", for \texttt{GET} requests.
%
We can explain this by imagining the memcached server as the "memory" of the middleware, the retrieval of a value still costs more than setting one.
%
Therefore we conclude that after some number of clients depending on the thread configuration the middleware becomes the bottleneck of the system.
%
\par
%
...
\subsubsection{Hypothesis}
%
We expect this configuration to handle twice as many requests as with one middleware before becoming saturated, assuming the server will not bottleneck the system.
%
...
\end{figure}
%
We can handle double the clients in this middleware before being saturated however still saturated
